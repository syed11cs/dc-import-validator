...
async def generate_sql_rule(body: _GenerateSqlRuleRequest):
    ...
    system_prompt = (
        ...
        "## Your task\n"
        "- `query`: a SELECT that returns the candidate rows.\n"
        ...
        "  query returns all (or filtered) rows; condition is a per-row boolean.\n"
        "  Example: 'all minimum values must be non-negative'\n"
        "  → SELECT StatVar, MinValue FROM stats  /  condition: MinValue >= 0\n\n"
        "Use an AGGREGATE rule when the description implies a count or global threshold:\n"
        "  keywords: 'zero', 'no rows', 'at least N', 'at most N', 'count', 'total', 'any'\n"
        "  query computes the aggregate; condition checks the aliased result.\n"
        "  Example: 'there should be zero StatVars with negative minimum values'\n"
        "  → SELECT COUNT(*) AS n FROM stats WHERE MinValue < 0  /  condition: n = 0\n\n"
        "Default to row-level. Only use aggregate when the description clearly implies a global count.\n\n"
        "## Condition rules (IMPORTANT)\n"
        "- The condition must reference only column names that appear in your SELECT list.\n"
        "- Use the MINIMAL condition that expresses the constraint. Do not add redundant checks.\n"
        "  If MaxValue < 0 implies MinValue < 0 for the same row, checking both is redundant.\n"
        "  WRONG: condition: MinValue < 0 AND MaxValue < 0\n"
        "  RIGHT: condition: MaxValue < 0\n"
        "- Aggregate functions (COUNT, SUM, AVG, MIN, MAX) are NEVER valid in the condition.\n"
        "  They must be aliased in the query and referenced by alias.\n"
        ...
        "Example 3 (aggregate): {\"query\": \"SELECT COUNT(*) AS n FROM stats WHERE MinValue < 0\", \"condition\": \"n = 0\"}"
    )
