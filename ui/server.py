...
async def generate_sql_rule(body: _GenerateSqlRuleRequest):
    ...
    system_prompt = (
        ...
        "Any row returned is a FAILURE. Empty result = PASSED.\n\n"
        "## Available tables\n\n"
        ...
        "  MeasurementMethods   TEXT    ← serialized list, e.g. '[]' when empty\n"
        "  Units                TEXT    ← serialized list, e.g. '[]' when empty\n"
        "  ScalingFactors       TEXT    ← serialized list, e.g. '[]' when empty\n"
        "  observationPeriods   TEXT    ← serialized list, e.g. '[P1Y, P1M]'\n\n"
        ...
        "DO NOT reference any column not listed above.\n"
        "DO NOT reference any other table.\n\n"
        "## Column semantics (CRITICAL)\n"
        "MinDate and MaxDate are TEXT ('YYYY-MM'). Do NOT use EXTRACT or date functions.\n"
        "  WRONG: EXTRACT(YEAR FROM MaxDate)\n"
        "  RIGHT: SUBSTRING(MaxDate, 1, 4) = '2024'\n"
        "  RIGHT: MaxDate >= '2020-01'\n\n"
        "Units, ScalingFactors, MeasurementMethods, observationPeriods store serialized\n"
        "Python list strings. An empty list is the string '[]', NOT NULL and NOT ''.\n"
        "  WRONG: condition: Units != ''     ← never matches; empty is '[]' not ''\n"
        "  RIGHT: condition: Units != '[]'   ← correct check for non-empty units\n\n"
        ...
        "  → query: SELECT StatVar, NumObservations FROM stats\n"
        "  → condition: NumObservations >= 1\n\n"
        "AGGREGATE: use when description implies a global count.\n"
        "Trigger words: 'zero', 'no rows', 'count', 'any', 'at least N', 'at most N', 'total'.\n"
        "Aggregate functions MUST be aliased; the alias is used in the condition.\n"
        "  'there should be zero StatVars with negative minimum values'\n"
        "  → query: SELECT COUNT(*) AS n FROM stats WHERE MinValue < 0\n"
        "  → condition: n = 0\n\n"
        "## Condition rules\n"
        "1. Use the MINIMAL sufficient condition. Do not add redundant checks.\n"
        "   WRONG: condition: MinValue >= 0 AND MaxValue >= 0\n"
        "   RIGHT: condition: MinValue >= 0\n\n"
        "2. Every condition must be meaningful. Never write trivially true conditions.\n"
        "   WRONG: condition: 1 = 1  or  condition: TRUE\n\n"
        "3. Condition must reference ONLY columns that appear in your SELECT list.\n\n"
        "4. Aggregate functions (COUNT, SUM, AVG, MIN, MAX) are forbidden in the condition.\n"
        "   Alias them in the query and reference by alias.\n"
        "   WRONG: condition: COUNT(*) = 0\n"
        "   RIGHT: query has COUNT(*) AS n, condition: n = 0\n\n"
        "## What NOT to generate\n"
        "- 'No duplicate StatVars': stats has exactly one row per StatVar — always trivially true.\n"
        "- Rules that select a column but add no meaningful constraint on it.\n"
        "  WRONG: SELECT StatVar, Units FROM stats  /  condition: 1 = 1\n\n"
        "## SQL constraints\n"
        ...
        "{\"query\": \"SELECT StatVar, MaxValue FROM stats WHERE Units != '[]'\", \"condition\": \"MaxValue <= 100\"}\n"
        "{\"query\": \"SELECT SUBSTRING(MaxDate, 1, 4) AS yr FROM stats\", \"condition\": \"yr >= '2020'\"}\n"
