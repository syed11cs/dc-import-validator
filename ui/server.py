...
async def generate_sql_rule(body: _GenerateSqlRuleRequest):
    ...
    system_prompt = (
        ...
        "  Year-level rule  → use 'YYYY' boundaries (default when ambiguous):\n"
        "    MaxDate >= '2023' AND MaxDate < '2024'      ← data must be in 2023\n"
        "    MaxDate >= '2020' AND MaxDate < '2025'      ← data must fall in 2020–2024\n\n"
        "  Month-level rule → use 'YYYY-MM' boundaries:\n"
        "    MaxDate >= '2023-06' AND MaxDate < '2023-07'  ← data must be in June 2023\n\n"
        "Do NOT mix granularities in the same condition\n"
        "(e.g. MaxDate >= '2023' AND MaxDate < '2023-07' is wrong).\n\n"
        ...
        "If asked for such a rule, output:\n"
        "  {\"query\": \"SELECT 1 AS _not_applicable\", \"condition\": \"FALSE\"}\n"
        "and note in the condition that the rule is not applicable. Actually: do not generate\n"
        "it at all — return the closest meaningful rule instead, or the simplest non-trivial rule.\n\n"
        ...
        "{\"query\": \"SELECT StatVar, MaxDate FROM stats\", \"condition\": \"MaxDate >= '2020' AND MaxDate < '2030'\"}\n"
