...
def _validate_custom_rules(custom_rules: list) -> str | None:
    ...
    _DUMMY_STATS = (
        ...
        " CAST(NULL AS DOUBLE) AS MeanValue,"
        " CAST(NULL AS DOUBLE) AS MedianValue,"
        " CAST(NULL AS DOUBLE) AS StdDev,"
        " CAST(NULL AS INTEGER) AS NumObservations,"
        " CAST(NULL AS TEXT) AS Units,"
        " CAST(NULL AS TEXT) AS ScalingFactor,"
        " CAST(NULL AS TIMESTAMP) AS EarliestDate,"
        " CAST(NULL AS TIMESTAMP) AS LatestDate,"
        " CAST(NULL AS TIMESTAMP) AS MaxDate LIMIT 0"
    )
    _DUMMY_DIFFER = (
...
async def generate_sql_rule(body: _GenerateSqlRuleRequest):
    ...
    system_prompt = (
        ...
        "## Context\n"
        "At runtime, your output will be used like:\n\n"
        "  WITH _data AS ({query})\n"
        "  SELECT * FROM _data WHERE NOT ({condition})\n\n"
        "Any row returned by that expression is a FAILURE.\n\n"
        "## Your task\n"
        "- `query`: a SELECT statement that returns the candidate rows.\n"
        "- `condition`: a boolean expression over the columns in your SELECT list that MUST BE TRUE for a row to PASS.\n\n"
        "Rows where the condition is false are failures.\n\n"
        "## Available tables\n"
        "Only two tables exist:\n\n"
        "Table `stats` (one row per StatVar):\n"
        "  StatVar TEXT, MinValue DOUBLE, MaxValue DOUBLE, MeanValue DOUBLE,\n"
        "  MedianValue DOUBLE, StdDev DOUBLE, NumObservations INTEGER,\n"
        "  Units TEXT, ScalingFactor TEXT,\n"
        "  EarliestDate TIMESTAMP, LatestDate TIMESTAMP, MaxDate TIMESTAMP\n\n"
        "Table `differ` (may be empty):\n"
        "  variableMeasured TEXT, ADDED INTEGER, DELETED INTEGER, MODIFIED INTEGER\n\n"
        "Do NOT reference any other table.\n\n"
        "## Choosing between row-level and aggregate\n\n"
        "Use a ROW-LEVEL rule when the check applies to each StatVar independently:\n"
        "  - query returns rows\n"
        "  - condition is per-row\n\n"
        "Example:\n"
        "  \"minimum values must be non-negative\"\n"
        "  → SELECT StatVar, MinValue FROM stats\n"
        "  → condition: MinValue >= 0\n\n"
        "Use an AGGREGATE rule when the description implies a global constraint.\n"
        "Keywords: \"zero\", \"no rows\", \"any\", \"count\", \"at least\", \"at most\", \"total\"\n\n"
        "Example:\n"
        "  \"there should be zero StatVars with negative minimum values\"\n"
        "  → SELECT COUNT(*) AS n FROM stats WHERE MinValue < 0\n"
        "  → condition: n = 0\n\n"
        "Default to row-level unless clearly aggregate.\n\n"
        "## Condition rules (IMPORTANT)\n"
        "- The condition must reference ONLY columns present in the SELECT list.\n"
        "- Use the MINIMAL condition required. Avoid redundancy.\n\n"
        "Example:\n"
        "  If MaxValue < 0 implies MinValue < 0, then:\n"
        "  WRONG: MinValue < 0 AND MaxValue < 0\n"
        "  RIGHT: MaxValue < 0\n\n"
        "- Aggregate functions (COUNT, SUM, etc.) are NOT allowed in the condition.\n"
        "  They must be aliased in the query.\n"
        "  WRONG: SELECT COUNT(*) FROM stats  /  condition: COUNT(*) = 0\n"
        "  RIGHT:  SELECT COUNT(*) AS n FROM stats  /  condition: n = 0\n\n"
        "## Column types (VERY IMPORTANT)\n"
        "- MinValue, MaxValue, MeanValue, MedianValue, StdDev → numeric\n"
        "- Units, StatVar, ScalingFactor → text\n"
        "- MaxDate, EarliestDate, LatestDate → TIMESTAMP\n\n"
        "Rules:\n"
        "- Do NOT use string functions (e.g. SUBSTRING) on TIMESTAMP columns.\n"
        "- To extract parts of a timestamp, use EXTRACT.\n\n"
        "Examples:\n"
        "  WRONG: SUBSTRING(MaxDate, 1, 4) = '2020'\n"
        "  RIGHT: EXTRACT(YEAR FROM MaxDate) = 2020\n\n"
        "## SQL constraints\n"
        ...
        "Row-level:          {\"query\": \"SELECT StatVar, MinValue FROM stats\", \"condition\": \"MinValue >= 0\"}\n"
        "Filtered row-level: {\"query\": \"SELECT StatVar, MaxValue FROM stats WHERE Units = 'Percent'\", \"condition\": \"MaxValue <= 100\"}\n"
        "Aggregate:          {\"query\": \"SELECT COUNT(*) AS n FROM stats WHERE MinValue < 0\", \"condition\": \"n = 0\"}"
    )
