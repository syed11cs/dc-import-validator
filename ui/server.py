...
async def generate_sql_rule(body: _GenerateSqlRuleRequest):
    ...
    system_prompt = (
        ...
        "When a rule specifies a numeric threshold (e.g. 'min value < 20000'), assess\n"
        "whether that threshold is meaningful across ALL StatVars before applying it globally.\n\n"
        "StatVars in a dataset can have vastly different scales — total population counts may\n"
        "be in the millions while subcategory counts are in the thousands. A single global\n"
        "threshold is likely wrong when the dataset contains StatVars of mixed magnitude.\n\n"
        "Decision:\n"
        "  - If the description clearly scopes the threshold to a named subset of StatVars\n"
        "    (e.g. 'infant deaths', 'under-1 mortality'), use ILIKE patterns to restrict the\n"
        "    query to that subset. Do NOT apply the threshold globally.\n"
        "    Example: 'infant deaths should be less than 20000'\n"
        "    → query: SELECT StatVar, MinValue FROM stats WHERE StatVar ILIKE '%Upto1Year%' OR StatVar ILIKE '%Infant%'\n"
        "    → condition: MinValue < 20000\n\n"
        "  - If the description applies the threshold globally without a clear semantic scope\n"
        "    (e.g. 'min value should be less than 20000'), the rule is ambiguous — different\n"
        "    StatVars will have different valid ranges.\n"
        "    → Return {\"error\": \"Rule applies a global numeric threshold that may not be\n"
        "       meaningful across StatVars with different scales. Specify which StatVars\n"
        "       the threshold applies to.\"}\n\n"
        "  - Thresholds that are universally valid regardless of scale (e.g. MinValue >= 0,\n"
        "    MaxValue <= 100 for percent StatVars) are safe to apply globally.\n\n"
