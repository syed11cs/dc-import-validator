...
async def generate_sql_rule(body: _GenerateSqlRuleRequest):
    ...
    system_prompt = (
        ...
        "  → query: SELECT StatVar, MaxValue FROM stats WHERE StatVar ILIKE '%Percent%' OR StatVar ILIKE '%Pct%'\n"
        ...
        "When a rule specifies a numeric threshold, ALWAYS assess whether it is meaningful\n"
        "across ALL StatVars before applying it globally. StatVars in a dataset can have\n"
        "vastly different scales (e.g. total births ~300K–3.6M vs infant deaths ~1K–20K).\n\n"
        "  SCOPED (description names a subset) → restrict with ILIKE, do NOT apply globally:\n"
        "    'infant deaths should be less than 20000'\n"
        "    → query: SELECT StatVar, MinValue FROM stats\n"
        "              WHERE StatVar ILIKE '%Death%Upto1Year%' OR StatVar ILIKE '%Death%Infant%'\n"
        "    → condition: MinValue < 20000\n\n"
        "  GLOBAL (no semantic scope given) → rule is ambiguous, return {\"error\"}:\n"
        "    'minimum value should be less than 500'\n"
        "    → {\"error\": \"This rule applies a numeric threshold across StatVars with very\n"
        "        different scales:\\n\"\n"
        "        \"  - Births (~300K–3.6M)\\n\"\n"
        "        \"  - Deaths (~240K–3.2M)\\n\"\n"
        "        \"  - Infant deaths (~1K–20K)\\n\\n\"\n"
        "        \"A threshold of 500 is not meaningful across all of them.\\n\"\n"
        "        \"Please specify a subset (e.g. infant deaths only).\",\n"
        "       \"suggestions\": [\n"
        "         \"minimum infant deaths should be less than 500\",\n"
        "         \"minimum value for a specific StatVar group should be less than 500\"\n"
        "       ]}\n\n"
        "  UNIVERSALLY SAFE (valid regardless of scale) → apply globally:\n"
        "    MinValue >= 0, MaxValue <= 100 for percent StatVars, NumObservations >= 1\n\n"
        ...
        "  1. Description is too vague (no column, threshold, or comparison can be inferred):\n"
        "       'values should look reasonable', 'data should be consistent',\n"
        "       'everything should be correct', 'check the data'\n"
        "  2. Rule requires comparing values across different StatVars (no join key available):\n"
        "       'Births should be greater than Deaths'\n"
        "       'value for StatVar A should exceed value for StatVar B'\n"
        "  3. Rule depends on observationPeriod or monthly/yearly frequency:\n"
        "       'monthly values should be less than yearly values'\n"
        "       'observations with P1M period should have more rows'\n"
        "  4. Rule applies a numeric threshold globally but StatVars have mixed scales:\n"
        "       'min value should be less than 20000'  ← ambiguous across all StatVars\n\n"
        "If any reasonable single-StatVar interpretation exists, generate SQL instead.\n"
        ...
        "When returning {\"error\"}, include ONLY \"error\" (and optionally \"suggestions\") —\n"
        "no \"query\" or \"condition\".\n\n"
        ...
        "Before emitting your response, check:\n"
        "  1. Does the rule apply to multiple StatVars with different value scales?\n"
        "     If yes — is the threshold meaningful across ALL of them? If not → {\"error\"}.\n"
        "  2. Is the condition a PASS test (not failure logic)?\n"
        "     'should not exceed 100' → MaxValue <= 100  (not MaxValue > 100).\n"
        "  3. Does the condition reference only columns in the SELECT list?\n"
        "  4. Is there any redundant filter or clause that can be removed?\n"
        "  5. Does the SQL exactly match what the description asks for?\n"
        "  6. Is the rule cross-StatVar or does it use observationPeriod? → {\"error\"}.\n\n"
        ...
        "{\"query\": \"SELECT StatVar, MaxValue FROM stats WHERE StatVar ILIKE '%Percent%' OR StatVar ILIKE '%Pct%'\", \"condition\": \"MaxValue <= 100\"}"
    )
