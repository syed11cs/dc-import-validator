...
async def generate_sql_rule(body: _GenerateSqlRuleRequest):
    ...
    system_prompt = (
        ...
        "  MinDate           TEXT    ← format 'YYYY-MM', e.g. '2020-01'\n"
        "  MaxDate           TEXT    ← format 'YYYY-MM', e.g. '2024-06'\n"
        "  MeasurementMethods TEXT\n"
        "  Units             TEXT\n"
        "  ScalingFactors    TEXT\n"
        "  observationPeriods TEXT\n\n"
        ...
        "## Column type rules (CRITICAL)\n"
        "MinDate and MaxDate are TEXT strings ('YYYY-MM'), NOT timestamps or dates.\n"
        "  WRONG: EXTRACT(YEAR FROM MaxDate)   ← MaxDate is TEXT, not a timestamp\n"
        ...
        "  → condition: Units IS NOT NULL AND Units != ''\n\n"
        ...
        "   MaxValue >= 0 already implies MinValue >= 0 for well-formed data — pick the tighter bound.\n"
        ...
        "- 'No duplicate StatVars': stats already has exactly one row per StatVar — this rule\n"
        "  is always trivially true. Do not generate it.\n"
        ...
        "{\"query\": \"SELECT StatVar, Units FROM stats\", \"condition\": \"Units IS NOT NULL AND Units != ''\"}\n"
        "{\"query\": \"SELECT StatVar, MaxValue FROM stats WHERE Units = 'Percent'\", \"condition\": \"MaxValue <= 100\"}\n"
